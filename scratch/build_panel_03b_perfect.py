...
draw.text((W // 2, 57), 'INSPECCIÓN Y MANTENIMIENTO DEL CAUCHO DE REPUESTO Y HERRAMIENTAS DE ELEVACIÓN', fill=(0, 242, 254), font=f_sub_title, anchor='mm')
...
draw.polygon(lid_poly, fill=(35, 42, 58), outline=(20, 26, 38))
for i in range(4):
    y_l = cy - 160 + i * 20
    draw.line([(cx - 190 + i * 12, y_l), (cx + 190 - i * 12, y_l)], fill=(24, 30, 42), width=3)
# Red pull strap
draw.polygon([(cx - 15, cy - 175), (cx + 15, cy - 175), (cx + 10, cy - 205), (cx - 10, cy - 205)], fill=(220, 40, 40), outline=(120, 10, 10))
draw.text((cx, cy - 192), 'TIRADOR', fill=(255, 255, 255), font=ImageFont.truetype(r'C:\Windows\Fonts\arialbd.ttf', 8), anchor='mm')

# Circular well depth
for d in range(35):
    draw.ellipse([cx - 260, cy - 105 + d, cx + 260, cy + 105 + d], fill=(16, 20, 30))
draw.ellipse([cx - 255, cy - 105 + 35, cx + 255, cy + 105 + 35], fill=(22, 28, 40), outline=(0, 242, 254), width=1)

# Paste Wheel graphic (from clean_spare_wheel_art):
...
if os.path.exists(wheel_alpha_path):
    ...
    panel.paste(w_img, (cx - 215, cy - 95), w_img)

# Scissor jack crop without English text:
im_kit = Image.open(r'c:\Users\Frode\Documents\Proyectos\CharuAutos\Ajustes\2.5 kit seguridad.jpg')
jack_clean = im_kit.crop((520, 240, 880, 385)).resize((190, 105), Image.Resampling.LANCZOS)
panel.paste(jack_clean, (cx + 45, cy - 40))
draw.rectangle([(cx + 44, cy - 41), (cx + 236, cy + 66)], outline=(0, 242, 254), width=1)
draw.text((cx + 140, cy - 28), 'GATO Y LLAVE DE RUEDAS', fill=(255, 204, 0), font=f_badge_t, anchor='mm')

# Inflator crop without English text:
inflator_clean = im_kit.crop((910, 245, 1120, 390)).resize((115, 105), Image.Resampling.LANCZOS)
panel.paste(inflator_clean, (cx + 120, cy + 80))
draw.rectangle([(cx + 119, cy + 79), (cx + 236, cy + 186)], outline=(0, 242, 254), width=1)
draw.text((cx + 177, cy + 92), 'MANOMETRO 12V', fill=(0, 242, 254), font=f_badge_t, anchor='mm')

# Callout Leaders with glowing lines:
# 1. Spare Tire Pressure
draw.line([(cx - 100, cy - 10), (cx - 170, cy - 55), (cx - 210, cy - 55)], fill=(0, 242, 254), width=2)
draw.rounded_rectangle([(cx - 275, cy - 70), (cx - 160, cy - 40)], radius=5, fill=(10, 16, 26), outline=(0, 242, 254), width=1)
draw.text((cx - 217, cy - 55), 'Caucho 60 PSI (Galleta)', fill=(0, 242, 254), font=f_badge_t, anchor='mm')

# 2. Central Anchor Bolt
draw.line([(cx - 70, cy + 30), (cx - 10, cy + 130), (cx + 30, cy + 130)], fill=(255, 204, 0), width=2)
draw.rounded_rectangle([(cx + 30, cy + 115), (cx + 110, cy + 145)], radius=5, fill=(10, 16, 26), outline=(255, 204, 0), width=1)
draw.text((cx + 70, cy + 130), 'Tornillo Anclaje', fill=(255, 204, 0), font=f_badge_t, anchor='mm')
